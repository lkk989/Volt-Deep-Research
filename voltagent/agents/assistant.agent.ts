...
export const assistantAgent = new Agent({
    ...
    name: 'Assistant',
    purpose:
        'Generate high-signal, source-targeted research queries and concise investigation plans for downstream specialist agents.',
    model: ({ context }) => {
    ...
    summarization: {
        ...
        maxOutputTokens: 800,
        systemPrompt:
            'Summarize the research conversation so far to preserve context.',
    },
            ...
            if (error) {
                const errMsg =
                    error instanceof Error
                        ...
                        : String(error)
                voltlogger.error(
                    `[${opId}] tool ${String(tool.name)} failed: ${errMsg}`
                )
            } else {
