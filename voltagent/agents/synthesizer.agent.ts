...
import z from 'zod'
import { researchAgentGuardrails } from '../config/guardrails.js'
...
const synthesizeInformationTool = createTool({
    ...
    description:
        'Combine and synthesize information from multiple sources into coherent insights',
    parameters: z.object({
        sources: z
            .array(z.string())
            .describe('Array of information sources to synthesize'),
        synthesisGoal: z
            .string()
            .describe('The goal or focus of the synthesis'),
    }),
    execute: ({ sources, synthesisGoal }, operationContext) => {
        ...
        voltlogger.info(
            `Synthesizing ${sources.length} sources for goal: ${synthesisGoal}`
        )
        ...
        commonThemes.forEach((theme) => {
            ...
            if (sourcesWithTheme.length >= sources.length * 0.5) {
                synthesis.keyThemes.push(
                    `${theme} (mentioned in ${sourcesWithTheme.length} sources)`
                )
            }
        })
        ...
        if (sources.some((s) => s.toLowerCase().includes('significant'))) {
            synthesis.integratedInsights.push(
                'Multiple sources indicate significant findings in the research area'
            )
        }

        if (sources.some((s) => s.toLowerCase().includes('further research'))) {
            synthesis.integratedInsights.push(
                'Sources consistently recommend additional research'
            )
        }
        ...
        for (const indicator of consensusIndicators) {
            if (!consensusFound) {
                ...
                if (sourcesWithConsensus.length > 0) {
                    synthesis.consensusPoints.push(
                        `${sourcesWithConsensus.length} sources indicate consensus on key points`
                    )
                    consensusFound = true
                }
            }
        ...
        if (sources.length < 3) {
            synthesis.gaps.push(
                'Limited number of sources may not provide comprehensive coverage'
            )
        }

        const methodologicalSources = sources.filter(
            (s) =>
        ...
        if (methodologicalSources.length === 0) {
            synthesis.gaps.push(
                'Limited methodological discussion across sources'
            )
        }
        ...
        if (synthesis.keyThemes.length > 3) {
            synthesis.recommendations.push(
                'Consider focusing on the most prominent themes for detailed analysis'
            )
        }
...
const resolveContradictionsTool = createTool({
    ...
    description:
        'Identify and resolve contradictions between different sources',
    parameters: z.object({
        conflictingInfo: z
            .array(z.string())
            .describe('Array of conflicting information pieces'),
        resolutionCriteria: z
            .string()
            .describe('Criteria for resolving contradictions'),
    }),
    execute: ({ conflictingInfo, resolutionCriteria }, operationContext) => {
        ...
        voltlogger.info(
            `Resolving contradictions using criteria: ${resolutionCriteria}`
        )
        ...
        if (conflictingInfo.length === 0) {
            resolution.unresolvedIssues.push(
        ...
        const contradictionPatterns = [
            ...
            {
                pattern: /versus|vs|compared to/i,
                type: 'Comparative contradiction',
            },
            {
                pattern: /on the other hand|conversely/i,
        ...
        conflictingInfo.forEach((info, index) => {
            contradictionPatterns.forEach(({ pattern, type }) => {
                if (pattern.test(info)) {
                    resolution.identifiedContradictions.push(
                        `${type} in source ${index + 1}: ${info.substring(0, 100)}...`
                    )
                }
            })
        })
        ...
        if (
            criteriaLower.includes('majority') ||
            criteriaLower.includes('most')
        ) {
            resolution.resolutionApproach = 'Majority Rule'
            resolution.resolvedPoints.push(
                'Using majority consensus to resolve contradictions'
            )
        } else if (
            criteriaLower.includes('recent') ||
            criteriaLower.includes('latest')
        ) {
            resolution.resolutionApproach = 'Recency'
            resolution.resolvedPoints.push(
                'Prioritizing most recent information'
            )
        } else if (
            criteriaLower.includes('authority') ||
            criteriaLower.includes('expert')
        ) {
            resolution.resolutionApproach = 'Authority'
            resolution.resolvedPoints.push(
                'Resolving based on source authority and credibility'
            )
        } else if (
            criteriaLower.includes('evidence') ||
            criteriaLower.includes('data')
        ) {
            resolution.resolutionApproach = 'Evidence-Based'
            resolution.resolvedPoints.push(
                'Resolving based on strength of evidence'
            )
        } else {
            resolution.resolutionApproach = 'Synthesis'
            resolution.resolvedPoints.push(
                'Attempting to synthesize conflicting viewpoints'
            )
        }
        ...
        if (resolution.identifiedContradictions.length === 0) {
            ...
            resolution.identifiedContradictions.length <=
            conflictingInfo.length * 0.3
        ) {
            resolution.confidence = 'Medium'
            resolution.resolvedPoints.push(
                'Minor contradictions resolved using specified criteria'
            )
        } else {
...
const createUnifiedNarrativeTool = createTool({
    ...
    description:
        'Create a unified, coherent narrative from disparate information',
    parameters: z.object({
        information: z
            .array(z.string())
            .describe('Array of information pieces to unify'),
        narrativeFocus: z
            .string()
            .describe('The central focus or theme of the narrative'),
    }),
    execute: ({ information, narrativeFocus }, operationContext) => {
        ...
        if (information.length === 0) {
            narrative.gaps.push(
                'No information provided for narrative creation'
            )
            return narrative
        }
        ...
        themeKeywords.forEach((theme) => {
            ...
            if (infoWithTheme.length >= information.length * 0.4) {
                narrative.themes.push(
                    `${theme} (present in ${infoWithTheme.length} information pieces)`
                )
            }
        })
        ...
        information.forEach((info, index) => {
            ...
            if (sentences.length > 0) {
                narrative.narrativeStructure.mainPoints.push(
                    `Key point ${index + 1}: ${sentences[0].trim()}`
                )
            }
        })
        ...
        if (information.length > 1) {
            narrative.narrativeStructure.connections.push(
                'Information sources show interconnected themes and complementary perspectives'
            )
            ...
            connectionWords.forEach((word) => {
                if (!foundConnection) {
                    ...
                    if (connectedInfo.length > 0) {
                        narrative.narrativeStructure.connections.push(
                            `${connectedInfo.length} sources use connecting language indicating relationships`
                        )
        ...
        if (totalConnections > 2) {
            ...
            narrative.gaps.push(
                'Limited connections between information pieces reduce narrative coherence'
            )
        }
...
export const synthesizerAgent = new Agent({
    ...
    name: 'Synthesizer',
    purpose:
        'Fuse multi-source outputs into a coherent, contradiction-resolved narrative with explicit confidence and open gaps.',
    model: ({ context }) => {
    ...
    instructions: synthesizerPrompt({
        ...
        tools: 'synthesis/contradiction/narrative tools, knowledge graph toolkit, RAG tools, workspace retrieval',
        standards:
            'Maintain intellectual honesty, preserve attribution, and explicitly report unresolved conflicts',
    ...
    tools: [
        ...
        resolveContradictionsTool,
        createUnifiedNarrativeTool,
        chunkContentRecursiveTool,
        chunkContentTokenTool,
        chunkContentSentenceTool,
        chunkContentTableTool,
    ...
    summarization: {
        ...
        maxOutputTokens: 800,
        systemPrompt:
            'Summarize the synthesis process. Focus on integrated insights, resolved contradictions, and the unified narrative created.',
    },
    ...
    outputGuardrails: researchAgentGuardrails.output,
})
