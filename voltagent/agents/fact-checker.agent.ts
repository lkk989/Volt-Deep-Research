...
export const factCheckerAgent = new Agent({
    ...
    name: 'Fact Checker',
    purpose:
        'Verify claims against credible evidence, detect bias/manipulation risk, and assign confidence for safe downstream synthesis.',
    model: ({ context }) => {
    ...
    instructions: factCheckerPrompt({
        ...
        sourceRequirements:
            'authoritative, recent, independent, and conflict-of-interest aware',
        ...
        standards:
            'Maintain objectivity, cite all sources, acknowledge uncertainty, and separate facts from interpretation',
    ...
    summarization: {
        ...
        maxOutputTokens: 800,
        systemPrompt:
            'Summarize the verification process. Focus on claims verified, confidence scores assigned, and any detected biases or gaps in evidence.',
    },
    ...
    hooks: {
        ...
        onToolEnd: async ({ tool, error, context }) => {
            ...
            if (error) {
                ...
                voltlogger.error(
                    `[${opId}] tool ${tool.name} failed: ${errorMessage}`
                )
                if (error instanceof Error) {
                    ...
                    if (typeof stack === 'string' && stack.length > 0) {
                        ...
                        voltlogger.debug(
                            `[${opId}] tool ${tool.name} stack: <no stack available>`
                        )
                    }
                }
            } else {
