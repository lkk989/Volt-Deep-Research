...
- **Advanced Chat Option Surface**: The dashboard chat flow now exposes typed AI SDK + VoltAgent request controls for generation parameters, memory persistence, semantic memory, provider-specific options, stop sequences, dynamic context JSON, and stream stop handling across the live agent chat route and shared chat components
- **Persona State Refinement**: `chat-messages.tsx` now maps the latest assistant persona state from real assistant activity (`reasoning`, active tool states, and subagent stream parts) instead of relying only on the top-level chat status, while preserving `idle`, `listening`, `thinking`, and `asleep` semantics
